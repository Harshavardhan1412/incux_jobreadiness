...
async function run200UserLoadTest() {
  console.log("===============================================================");
  console.log(`?? READYSETJOB: 200-CANDIDATE EXAM SUBMISSION CONCURRENCY BENCHMARK`);
  console.log(`Target: ${TOTAL_USERS} Simultaneous Candidates Submitting Code`);
  console.log(`Timestamp: ${new Date().toISOString()}`);
  console.log("===============================================================\n");

  // --- Phase 1: High-Concurrency Submission Burst -----------------------------
  console.log(`[Phase 1] Simulating ${TOTAL_USERS} candidates submitting code simultaneously...`);
  ...
  for (let i = 1; i <= TOTAL_USERS; i++) {
    ...
    const payload = {
      ...
      questionId: "q-exam-challenge-200",
      language: profile.language,
      sourceCode: profile.sourceCode,
      testCases: profile.testCases
    };

    // Stagger submission slightly (0-1500ms) to simulate real human closing-second clicks
  ...
  if (successful202s.length !== TOTAL_USERS) {
    console.error("?? Notice: Some submissions were not accepted with HTTP 202.");
  } else {
    console.log(`? 100% of ${TOTAL_USERS} candidates received immediate HTTP 202 Accepted! Gateway did not drop a single request.\n`);
  }
  ...
  while (completedCount < activeJobs.length && (Date.now() - trackingStart) < 240000) {
  ...
  console.log(`Total Time to Drain All ${TOTAL_USERS}   : ${(totalDrainTime / 1000).toFixed(2)} seconds`);
  ...
  if (completionDurations.length > 0) {
    ...
    console.log(`p25 (First 50 students done)        : ${p25Turnaround}s`);
    console.log(`p50 Median (100 students finished)  : ${p50Turnaround}s`);
    console.log(`p75 (150 students finished)         : ${p75Turnaround}s`);
    console.log(`p90 (180 students finished)         : ${p90Turnaround}s`);
    console.log(`p95 (190 students finished)         : ${p95Turnaround}s`);
    console.log(`Max (All 200 students completed)    : ${maxTurnaround}s`);
  ...
  console.log("\n===============================================================");
  console.log(`?? VERDICT: The platform successfully handled ${TOTAL_USERS} simultaneous candidates!`);
  console.log("===============================================================");
}

run200UserLoadTest().catch(console.error);
