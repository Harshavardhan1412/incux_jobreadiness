import { mockPeerComparison, COLORS } from '../../data/mockData';
...
export default function PeerComparison({ student }) {
  ...
  return (
    <section id="peers" className="space-y-6">
      <h2 className="text-xl font-bold text-slate-900">Peer Comparison</h2>

      {/* Overall Stats */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-blue-50 rounded-2xl p-5 ring-1 ring-blue-200">
          <p className="text-sm font-medium text-slate-500">Your Average</p>
          <p className="text-3xl font-bold text-blue-700">{overallStudent}%</p>
        ...
        <div className="bg-slate-50 rounded-2xl p-5 ring-1 ring-slate-200">
          ...
          <p className="text-3xl font-bold text-slate-700">{overallAvg}%</p>
        ...
        <div className="bg-purple-50 rounded-2xl p-5 ring-1 ring-purple-200">
          <p className="text-sm font-medium text-slate-500">Topper Score</p>
          <p className="text-3xl font-bold text-purple-700">{overallTopper}%</p>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
        <h3 className="text-lg font-semibold text-slate-800 mb-4">Category-wise Comparison</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="text-left py-3 px-4 font-semibold text-slate-600">Category</th>
                <th className="text-center py-3 px-4 font-semibold text-blue-600">You</th>
                <th className="text-center py-3 px-4 font-semibold text-slate-500">Average</th>
                <th className="text-center py-3 px-4 font-semibold text-purple-600">Topper</th>
                <th className="text-center py-3 px-4 font-semibold text-slate-500">Median</th>
                <th className="text-center py-3 px-4 font-semibold text-slate-600">Your Percentile</th>
              </tr>
            </thead>
            <tbody>
              {data.map((d, i) => {
                ...
                return (
                  <tr key={i} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    <td className="py-3 px-4 font-medium text-slate-700">{d.category}</td>
                    <td className="py-3 px-4 text-center">
                      <span className="bg-blue-100 text-blue-700 px-2.5 py-1 rounded-full font-semibold">
                        {d.studentScore}%
                      </span>
                    </td>
                    <td className="py-3 px-4 text-center text-slate-500">{d.classAverage}%</td>
                    <td className="py-3 px-4 text-center text-purple-600 font-medium">{d.topperScore}%</td>
                    <td className="py-3 px-4 text-center text-slate-400">{d.classMedian}%</td>
                    <td className="py-3 px-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <span className="text-xs font-semibold text-slate-600">~{cappedPercentile}th</span>
                      </div>
                    </td>
                  </tr>
                );
