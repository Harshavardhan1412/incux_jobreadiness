import { COLORS, getCategoryPercents } from '../../data/mockData';
import ScoreBarChart from '../charts/BarChart';
import ScoreLineChart from '../charts/LineChart';
...
export default function ScoreOverview({ student }) {
  ...
  const bestEntry = Object.entries(currentPercents).sort((a, b) => b[1] - a[1])[0];
  ...
  return (
    <section id="overview" className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Overall Score" value={`${student.overallScore}%`} change={change} color="blue" />
        <StatCard label="Percentile" value={`${student.percentile}th`} change={null} color="emerald" />
        <StatCard label="Rank" value={`${student.rank}/${student.totalStudents}`} change={null} color="purple" />
        <StatCard
          label="Best Category"
          value={bestEntry ? bestEntry[0].charAt(0).toUpperCase() + bestEntry[0].slice(1) : '—'}
          change={null}
          color="amber"
          subtitle={bestEntry ? `${bestEntry[1]}%` : undefined}
        />
      </div>
...
function StatCard({ label, value, change, color, subtitle }) {
  ...
  return (
    <div className={`${c.bg} rounded-2xl p-5 ring-1 ${c.ring}`}>
      <p className="text-sm font-medium text-slate-500 mb-1">{label}</p>
      <p className={`text-2xl font-bold ${c.text}`}>{value}</p>
      {subtitle && <p className="text-sm text-slate-500 mt-1">({subtitle})</p>}
      {change !== null && (
        <p className={`text-xs font-medium mt-2 ${change >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
